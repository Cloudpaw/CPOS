...
The stuff that only big city lawyers can read:
...
OSReloadSettings = function()
	...
	desktop:Draw()
	OSSettings = OSTableIO.load("Settings")
...
--essentially, what this does is sets what application should open certain file types--

	list = {
		txt = "System/Applications/TextEdit.app"
	},

	register = function(extention, applicationPath) --return false if the register fails
		--TODO: prompt the user on what application they want for an extension
		if not OSExtensionAssociations.list[extension] then --prevent the extension from being overwritten
			OSExtensionAssociations.list[extention] = applicationPath
			return true
		else
			return false
		end
...
function OSHandleKeystroke (keystroke) 
	...
	elseif #name == 1 then -- a character
...
function initMenuBar()
	local menuItems = {
		OSMenuItem:new("About this Pear", function() Finder.environment:aboutSystem() end),
		OSMenuItem:new("Software Update", nil), --TODO: add software update
		OSMenuItem:new("App Store", nil), --TODO: add app store
		OSMenuSplitter:new(),
		OSMenuItem:new("System Preferences", function() 
												local systemPreferences = OSApplication:load("System/Applications/System Preferences.app")
												OSApplication.run(systemPreferences)
											end),
		OSMenuSplitter:new(),
		OSMenuItem:new("Switch Screen", function() OSServices.switchScreen() end),
		OSMenuItem:new("Enter CraftOS", function() OSServices.craftos() end),
		OSMenuItem:new("Sleep", function() OSServices.sleep() end),
...
function initDock()
	local dockItems = {}
	local applicationsPath = "System/Applications/"
...
function firstRun()
	...
	OSFirstRunMode = true
	desktop = OSDesktop:new()
	FirstRun = OSApplication:load("System/Library/FirstRun.app")
